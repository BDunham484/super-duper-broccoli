...
In Module 18, you will introduce students to NoSQL databases and performing CRUD operations using MongoDB, a popular NoSQL document-based database. 

Students will also learn how to use Mongoose, an Object Data Modeling (ODM) library, to enforce a specific schema on data and handle data relationships. First, make sure that you first have both MongoDB and Node.js installed. We will be using Mongoose version `6.9.2`, as version 7+ potentially introduces breaking changes for our activities. For start-up instructions, refer to the [Mongoose docs on getting started](https://mongoosejs.com/docs/index.html#).

Deployment of an application using Mongo Atlas and Heroku is also discussed.
